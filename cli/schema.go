...
// designed for agent introspection via lnget schema.
type CommandSchema struct {
	// Name is the command name (e.g. "tokens").
	Name string `json:"name"`

	// FullPath is the fully qualified command path (e.g. "lnget tokens list").
...
var exitCodeDocs = map[int]string{
	...
	ExitInvalidArgs:  "invalid arguments or payment too expensive",
	ExitPaymentFailed: "L402 payment failed",
	ExitNetworkError: "network or connection error",
	ExitAuthFailure:  "authentication failure",
...
func NewSchemaCmd() *cobra.Command {
  ...
  lnget schema tokens     Show schema for the tokens command
  lnget schema --all      Full schema tree`,
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			// Walk up to the root command.
			root := cmd.Root()
...
// argument descriptions. Cobra's Use format is "name [flags] <arg1>
// [arg2]" where <> means required and [] means optional.
	...
	for _, p := range parts[1:] {
		...
		if strings.HasPrefix(p, "<") && strings.HasSuffix(p, ">") {
			...
			args = append(args, ArgSchema{
				Name:        name,
				Required:    true,
				Description: fmt.Sprintf("Required: %s", name),
			})
			...
			args = append(args, ArgSchema{
				Name:        name,
				Required:    false,
				Description: fmt.Sprintf("Optional: %s", name),
			})
		}
	}
...
// individual command names for cmd.Find(). Supports both
// "tokens.list" and "tokens list" formats.
